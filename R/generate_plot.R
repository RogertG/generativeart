...
generate_plot <- function(df, file_name, polar, filetype, color = "black", background_color = "white") {
  ...
  if (polar == TRUE) {
    plot <- df %>%
      ...
      ggplot2::geom_point(alpha = 0.4, size = 0, shape = 46, color = color) +
      ggplot2::theme_void() +
      ggplot2::coord_fixed() +
      ggplot2::coord_polar() +
      ggplot2::theme(
  ...
  ggplot2::ggsave(plot, filename = paste0(IMG_PATH, file_name), width = 15, height = 15, dpi = 300, device = filetype)
  print("image saved...")
}
